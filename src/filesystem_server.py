# MCP Filesystem Python server curtesy of "Punkeye" https://github.com/punkpeye/mcp-filesystem-python
import asyncio
import os
import sys
...
class FileDelete(BaseModel):
    ...
    recursive: Annotated[
        bool,
        Field(default=False, description="Recursively delete directories"),
    ]
...
def is_ignored(
    root_path: str, path: str, ignore_patterns: pathspec.PathSpec | None
) -> bool:
...
async def serve(
    root_path: str, custom_ignore_patterns: list[str] | None = None
) -> None:
    ...
    async def handle_get_prompt(
        ...
        try:
            ...
            raise McpError(
                types.ErrorData(
                    code=types.INVALID_PARAMS,
                    message=f"File is not text-based:{path}",
                )
            )
        except IOError:
            raise McpError(
    ...
    async def handle_call_tool(
        ...
        if not arguments:
            raise McpError(
                types.ErrorData(
                    code=types.INVALID_PARAMS, message="Missing arguments"
                )
            )
        ...
        if name == "search-files":
            ...
            for root, _, files in os.walk(root_path):
                for file in files:
                    ...
                    include_patterns = pathspec.PathSpec.from_lines(
                        "gitwildmatch", [args.file_pattern]
                    )
                    if not include_patterns.match_file(file):
                        continue
